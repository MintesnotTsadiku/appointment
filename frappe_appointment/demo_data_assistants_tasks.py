...
def generate_va_profiles(count=5):
    ...
    try:
        ...
        for i in range(count):
            ...
            va_profile.bio = f"Experienced virtual assistant specializing in administrative and customer service tasks."
            va_profile.education = f"Bachelor's degree in Business Administration"
            ...
            # Add skills
            if skills:
                selected_skills = random.sample(skills, k=min(3, len(skills)))
...
def generate_assignments(count=8):
    ...
    try:
        ...
        # Track assignments per VA
        va_assignments = {va["name"]: [] for va in va_profiles}
        
        for i in range(count):
            # Find a VA that can take more clients
            available_vas = [
            ...
            if va["max_clients"] == 1:
                ...
                priority = "primary" if len(va_assignments[va["name"]]) == 0 else "secondary"
            else:  # max_clients == 3
...
def generate_task_projects(count=5):
    ...
    try:
        ...
        for i in range(min(count, len(project_names))):
            ...
            project.status = random.choice(["planning", "in_progress", "on_hold"])
...
def generate_tasks(count=20):
    ...
    try:
        ...
        for i in range(count):
            ...
            task.status = random.choice(["requested", "assigned", "in_progress", "completed"])
            ...
            if random.choice([True, False]):
                ...
                task.deadline = datetime.now() + timedelta(days=days_ahead)
                # Mark for daily briefing if deadline is today or soon
                if 0 <= days_ahead <= 3:
