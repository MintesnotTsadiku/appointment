...
from frappe_appointment.scheduler.doctype.booking_event.booking_event import get_events
...
class TestEvent(IntegrationTestCase):
	def setUp(self):
		frappe.db.delete("Booking Event")
		make_test_objects("Booking Event", reset=True)
	...
	def test_allowed_public(self):
		...
		doc = frappe.get_doc("Booking Event", frappe.db.get_value("Booking Event", {"subject": "_Test Event 1"}))
		self.assertTrue(frappe.has_permission("Booking Event", doc=doc))

	def test_not_allowed_private(self):
		frappe.set_user(self.test_user)
		doc = frappe.get_doc("Booking Event", frappe.db.get_value("Booking Event", {"subject": "_Test Event 2"}))
		self.assertFalse(frappe.has_permission("Booking Event", doc=doc))

	def test_allowed_private_if_in_event_user(self):
		name = frappe.db.get_value("Booking Event", {"subject": "_Test Event 3"})
		frappe.share.add("Booking Event", name, self.test_user, "read")
		frappe.set_user(self.test_user)
		doc = frappe.get_doc("Booking Event", name)
		self.assertTrue(frappe.has_permission("Booking Event", doc=doc))
		frappe.set_user("Administrator")
		frappe.share.remove("Booking Event", name, self.test_user)

	def test_event_list(self):
		frappe.set_user(self.test_user)
		res = frappe.get_list(
			"Booking Event", filters=[["Booking Event", "subject", "like", "_Test Event%"]], fields=["name", "subject"]
		)
	...
	def test_revert_logic(self):
		ev = frappe.get_doc(self.globalTestRecords["Booking Event"][0]).insert()
		name = ev.name

		frappe.delete_doc("Booking Event", ev.name)
	...
	def test_assign(self):
		...
		ev = frappe.get_doc("Booking Event", ev.name)
