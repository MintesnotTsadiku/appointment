...
frappe.ui.form.on('Configuration Settings', {
	refresh: function(frm) {
		...
		// Clear any existing Assistants & Tasks auto-refresh timer
		if (frm._at_auto_refresh_timer) {
			clearTimeout(frm._at_auto_refresh_timer);
			frm._at_auto_refresh_timer = null;
		}
		...
		// Auto-refresh Assistants & Tasks log every 3 seconds ONLY when generation is actively in progress
		if (frm.doc.at_demo_generation_log) {
			const log = frm.doc.at_demo_generation_log;
			const is_starting = log.includes('Starting') || log.includes('Generating') || log.includes('Creating');
			const is_complete = log.includes('completed') || log.includes('Finished') || log.includes('Error') || log.includes('success');
			...
			if (is_starting && !is_complete) {
				// Set a flag to prevent multiple timers
				if (!frm._at_is_refreshing) {
					frm._at_is_refreshing = true;
					frm._at_auto_refresh_timer = setTimeout(() => {
						frm._at_is_refreshing = false;
						frm.reload_doc();
					}, 3000);
				}
			} else {
	...
	btn_generate_at_demo_data: function(frm) {
		// Show confirmation dialog
		frappe.confirm(
			'Generate Assistants & Tasks demo data with the selected options?',
			() => {
				// User confirmed
				frappe.show_alert({
					message: __('Starting Assistants & Tasks demo data generation...'),
					indicator: 'blue'
				}, 5);
				...
				frm.reload_doc().then(() => {
					// Set the button field value and save
					frm.set_value('btn_generate_at_demo_data', 1);
					frm.save().then(() => {
						// Reload after 3 seconds to show initial log
						// The refresh handler will continue auto-refreshing if needed
						setTimeout(() => {
			...
			() => {
				// User cancelled
				frappe.show_alert({
					message: __('Assistants & Tasks demo data generation cancelled'),
					indicator: 'orange'
				}, 3);
			}
		);
	...
	btn_delete_at_demo_data: function(frm) {
		// Show warning dialog
		frappe.warn(
			'Delete Assistants & Tasks Demo Data?',
			'This will permanently delete all Assistants & Tasks demo data (skills, categories, templates, profiles, assignments, projects, tasks). This action cannot be undone.',
			() => {
				// User confirmed deletion
				frappe.show_alert({
					message: __('Starting Assistants & Tasks demo data deletion...'),
					indicator: 'orange'
				}, 5);
				...
				frm.reload_doc().then(() => {
					// Set the button field value and save
					frm.set_value('btn_delete_at_demo_data', 1);
					frm.save().then(() => {
						// Reload after 3 seconds to show initial log
						// The refresh handler will continue auto-refreshing if needed
						setTimeout(() => {
