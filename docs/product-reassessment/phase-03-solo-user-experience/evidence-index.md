...
`frappe-appointment-readiness` @ `3c57fb4` (unchanged). Documentation HEAD before
this follow-up: `67f6b59`.
...
The original F2 wording ("never reaches the schedule") is superseded by the
follow-up evidence below.
...
| BQA-2026-00090 | handoff (booking, exists, reception, calendar) in one run | Booking passed; record not shown in reception/calendar | `appointment-p3b-handoff-booking-...T201938Z`, `-exists-...T201959Z`, `-provider-reception-...T202005Z`, `-provider-calendar-...T202016Z` |
...
|---|---|---|
| F1 onboarding contract | BQA-2026-00094; `p3b-13-onboarding-after-skip.png`, `p3b-14-onboarding-create-service-error.png`; `traces/p3b-onboarding-skip-report.json`; `probes/outputs/p3b-desk-http.txt` (unrelated, see F3) | Availability save fails; Skip reaches Create Service; `create_service` 417 "Location not found" |
| F2 live-booking handoff | BQA-2026-00090; `p3b-01/02/03`; `traces/p3b-handoff-*.json`; `trace.zip` and `fixture_cleanup` contain `BEV00005` | Record present during inspection (same run); absent from reception and calendar |
| F3 update defect | BQA-2026-00087/88; `p3b-04..07`; `traces/p3b-update-*.json`; `probes/outputs/p3b-desk-http.txt`; `p3b-verify-outcomes.txt` | Causal: unpadded hour parse; two-digit control persists |
...
| F5 booking detail errors | `p3b-01`, `p3b-16`, original `screenshots/10`,`13`; source `organization-appointment/index.tsx:240-241`; `probes/outputs/p3b-desk-http.txt` (empty site timezone) | Duration "0.5 min", Service = org name, timezone = browser |
...
1. `appointment.qa_runner.run` drops `capture_video`/`capture_instruction_timeline`
   (all runs `video_file: null`); prerequisite is a one-line forward or API default
   change. Traces are not video.
...
3. Runner cleanup deletes new Booking Events by snapshot; no preserve facility, so
   cross-run handoff is impossible without a code change.
4. Product-API cleanup blocked by the paused, overloaded job queue.

## Runtime and tooling notes

- Runtime already running; not restarted, migrated or reset. Email muted and
...
All original screenshots, reports, outputs and probes are unchanged. The
corrected conclusions are additive; the superseded F2 wording is marked above.
