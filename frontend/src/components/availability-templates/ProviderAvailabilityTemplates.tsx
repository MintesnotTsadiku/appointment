...
interface ProviderAvailabilityTemplatesProps {
  schedule: DaySchedule[];
  onChange: (schedule: DaySchedule[]) => void;
  parentSchedule?: DaySchedule[]; // Service schedule to inherit from (preferred), or Location schedule as fallback
  serviceName?: string; // Service name (preferred parent)
  locationName?: string; // Location name (fallback parent)
...
export const ProviderAvailabilityTemplates = ({ 
  schedule, 
  onChange, 
  parentSchedule,
  serviceName,
  ...
  timeFormat = '12h',
  onUseDefaultChange
}: ProviderAvailabilityTemplatesProps) => {
  ...
  const [startOffset, setStartOffset] = useState(0); // minutes to add to start time
  const [endOffset, setEndOffset] = useState(0); // minutes to subtract from end time
  ...
  /**
   * Copy location schedule directly
   */
  ...
  /**
   * Copy location schedule with offset adjustments
   * - startOffset: positive = start later, negative = start earlier (but not before location)
   * - endOffset: positive = end earlier, negative = end later (but not after location)
   */
  const applyOffsetFromLocation = () => {
    ...
    const newSchedule: DaySchedule[] = parentSchedule.map((daySchedule) => {
      ...
      // Get location boundaries for this day
      ...
      const newRanges = daySchedule.ranges.map((range) => {
        // Apply start offset (positive = later only, cannot be earlier than location)
        // Start must be >= location start (allowEarlier=false)
        const newStart = addMinutesToTime(range.start, startOffset, locationStart, range.end, false);
        
        // Apply end offset (positive = earlier, cannot be later than location)
        // End cannot exceed location end (maxTime constraint enforced)
        const newEnd = addMinutesToTime(range.end, -endOffset, newStart, locationEnd, false);
  ...
  return (
    <div className="space-y-4 mb-6">
      {/* Inherit from Service (or Location as fallback) */}
      {hasParentSchedule && (
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-white">
                {serviceName ? (
                  <>Inherit from Service <span className="text-sm font-normal text-gray-500">({serviceName})</span></>
                ) : (
            ...
            {isInheritOpen && (
              ...
              >
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  {serviceName 
                    ? "Copy the service's availability directly, or adjust with time offsets."
                ...
                <div className="space-y-4">
                  ...
                  >
                    ...
                    <div className="text-left">
                      <div className="font-medium">
                        {serviceName ? "Copy Service Hours Exactly" : "Copy Location Hours Exactly"}
                      </div>
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        {serviceName 
                          ? "Use the same schedule as the service"
                  ...
                  <div className="p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
                    <h4 className="text-sm font-medium text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                      ...
                      {serviceName ? "Offset from Service Hours" : "Offset from Location Hours"}
                    </h4>
                    ...
                    <div className="mb-4">
                      <label className="block text-sm text-gray-600 dark:text-gray-400 mb-2">
                        End Time: <span className="font-medium text-gray-900 dark:text-white">
                          {endOffset === 0 ? (serviceName ? 'Same as service' : 'Same as location') : 
                           endOffset > 0 ? `${endOffset} min earlier` : `${Math.abs(endOffset)} min later`}
                        </span>
                        <span className="text-xs text-gray-500 ml-2">(Cannot exceed {serviceName ? 'service' : 'location'} closing)</span>
                      </label>
                      <div className="flex items-center gap-2">
                        <Button
                          variant="outline"
      ...
      <Card className="p-4 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 border-purple-200 dark:border-purple-800">
        ...
        <AnimatePresence initial={false}>
          {isQuickTemplatesOpen && (
            ...
            >
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                Apply a schedule to all weekdays. Provider hours must be within location hours.
              </p>
