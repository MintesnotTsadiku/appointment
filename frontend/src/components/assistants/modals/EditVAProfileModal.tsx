...
interface EditVAProfileModalProps {
  vaId: string | null;
  isOpen: boolean;
  onClose: () => void;
  onSave?: () => void;
...
export const EditVAProfileModal = ({ vaId, isOpen, onClose, onSave }: EditVAProfileModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await assistantAPI.vaProfile.update(vaId!, formData);
      if (response.success) {
      ...
      toast.error(error?.message || 'Failed to update VA profile');
    } finally {
      setLoading(false);
    }
  };
