...
import { Plus, Loader2, User, Mail, Phone, Globe, Briefcase } from 'lucide-react';
...
interface CreateVAProfileModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess?: () => void;
}

export const CreateVAProfileModal = ({ isOpen, onClose, onSuccess }: CreateVAProfileModalProps) => {
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState<Partial<VAProfile>>({
    full_name: '',
    email: '',
    phone: '',
    status: 'active',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await assistantAPI.vaProfile.create(formData);
      if (response.success) {
      ...
      toast.error(error?.message || 'Failed to create VA profile');
    } finally {
      setLoading(false);
    }
  };
